...
void CAntLoop::UpdateAnt(CAnt &ant)
{
    ...
    if(!m_HillAlreadyFound)
    {
        ...
        if(((ant.GetAntPosX() == ant.GetAntGoalX()) && (ant.GetAntPosY() == ant.GetAntGoalY())) || !ant.GetGoalReachable())
        {
            ...
            //пытаемся обнаружить муравейник
            ...
            if(m_Map.Cell(ant.GetAntPosX(), ant.GetAntPosY()) == McHill)
            {
                ...
                if (!stickMessageGo.empty())
                {
                    std::cout<<"glad"<<stickMessageGo[0]<<"  "<<stickMessageGo[1]<<std::endl;
...
# endif
    ...
    std::string dynamicView = m_State.GetVisibleDynMap();

   // здесь надо модифицировать m, если заметили охотника
