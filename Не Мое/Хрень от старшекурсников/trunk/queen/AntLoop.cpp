...
void CAntLoop::UpdateAnt(CAnt &ant)
{
    ...
    //3) символы, которые видит муравей на статической и динамической картах.
        ...
        // если видим муравейник, то идем к нему
        int minX, minY;
        int GetAntGoalX = ant.GetAntGoalX(), GetAntGoalY = ant.GetAntGoalY();
        bool messageFind = false;
        size_t numberOfMessage;

        if(!m_Board.GetMessage(BkStick).empty())
        {
            std::vector<int> Stick = m_Board.GetMessage(BkStick);

            for(numberOfMessage = 0; numberOfMessage < m_Board.GetMessage(BkStick).size();numberOfMessage += 2)
            {
                int x = Stick[numberOfMessage], y = Stick[numberOfMessage + 1];
                int minLenght = 100, lenght;


                if (x < GetAntGoalX && y < GetAntGoalY)
                {
                        for (x = Stick[numberOfMessage]; x = GetAntGoalX ; x--)
                    {
                        lenght = lenght + 1;
                     }
                        x = Stick[numberOfMessage];
                        for (y = Stick[numberOfMessage +1]; y = GetAntGoalY ; y--)
                    {
                         lenght = lenght + 1;
                    }
                        y = Stick[numberOfMessage + 1];
                         if(minLenght > lenght)
                    {
                         minLenght = lenght;
                         minX = x;
                ...
                if (x >= GetAntGoalX && y < GetAntGoalY)
                {
                        for (x = GetAntGoalX; x = Stick[numberOfMessage]; x++)
                    {
                        lenght = lenght + 1;
                     }
                        x = Stick[numberOfMessage];
                ...
                if (x >= GetAntGoalX && y >= GetAntGoalY)
                {
                        for (x = GetAntGoalX; x = Stick[numberOfMessage]; x++)
                    {
                        ...
                        x = Stick[numberOfMessage];
                        for (y = GetAntGoalY ; y = Stick[numberOfMessage +1]; y--)
                    {
                         lenght = lenght + 1;
                    }
                        y = Stick[numberOfMessage + 1];
               ...
               // FindPath(m_Map, x, y, hillX, hillY);
            }
            std::vector<int> queenMessage;
            queenMessage.push_back(minX);
            queenMessage.push_back(minY);
            m_Board.AddMessage(queenMessage, BkQueenCommand);
    ...
    if (!hillFound)
        ...
        // вначале считаем, что цель достижима
        ant.SetGoalReachable(true);
    ...
    // осталось поместить свои сообщения на доску объявлений:
    // очищаем все, что пришло от сервера.
