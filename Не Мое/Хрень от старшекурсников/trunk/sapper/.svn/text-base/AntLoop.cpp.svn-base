...
bool SearchFog(const CMap &map, int startX, int startY, int *endX, int *endY)
{
    bool AvailableTop = true, AvailableLeft = true, AvailableDown = true, AvailableRight = true;
    int col = map.GetMapSizeX();
    int row = map.GetMapSizeY();
    int radius = 3;
    int maxSize;
    int x , y;
    CMap searchingMap = map;
    if (col >= row)
    {
        maxSize = col;
    }
    ...
    for (radius = 3; radius <= maxSize; radius++)
    {
        if (AvailableTop)
        {
            y = startY - radius;
            ...
            {
                if ((searchingMap.Cell(x, y) == McUnknown) && FindPath(searchingMap, startX, startY, x, y))
                {
                    *endX = x;
                    *endY = y;
                    return true;
            ...
            if (searchingMap.Cell(x, y) == McWall)
            {
                AvailableTop = false;
            }
        }
    ...
    return false; // если вся карта разведана, то возвращаем false
}
...
void CAntLoop::UpdateAnt(CAnt &ant)
{
    ...
    if(((ant.GetAntPosX() == ant.GetAntGoalX()) && (ant.GetAntPosY() == ant.GetAntGoalY())) || !ant.GetGoalReachable())
    {
       ...
       // std::vector<int> foodMessage = m_Board.GetMessage(BkFood);
        ...
        int fogX, fogY;
        SearchFog(m_Map, ant.GetAntPosX(), ant.GetAntPosY(), &fogX, &fogY);


        ant.SetAntGoalX(fogX);
        ant.SetAntGoalY(fogY);
        for (int y = 0; y <= m_Map.GetMapSizeY(); y++)
        {
            for (int x = 0; x <= m_Map.GetMapSizeX(); x++)
            {
                // Ищем ловушки
                if (m_Map.Cell(x, y) == McTrap)
                {
    ...
    std::vector<int> foodMessage;
    std::vector<int> trapMessage;
    ...
    for (int y = 0; y <= m_Map.GetMapSizeY(); y++)
    {
        for (int x = 0; x <= m_Map.GetMapSizeX(); x++)
        {
            ...
            if (m_Map.Cell(x, y) == McStick)
            {
                foodMessage.push_back(x);
                foodMessage.push_back(y);
            }

            if (m_Map.Cell(x, y) == McStick)
            {
                trapMessage.push_back(x);
                trapMessage.push_back(y);
            }
        }
    }

    m_Board.AddMessage(stickMessage, BkStick);
    m_Board.AddMessage(foodMessage, BkStick);
    m_Board.AddMessage(trapMessage, BkStick);
