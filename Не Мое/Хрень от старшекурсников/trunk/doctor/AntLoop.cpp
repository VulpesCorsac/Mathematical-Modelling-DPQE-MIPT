...
void CAntLoop::UpdateAnt(CAnt &ant)
{
    ...
    if(((ant.GetAntPosX() == ant.GetAntGoalX()) && (ant.GetAntPosY() == ant.GetAntGoalY())) || !ant.GetGoalReachable())
    {
       ...
       std::vector<int> IllMessage = m_Board.GetMessage(BkIll);
    ...
    //{
        std::vector<int> IllMessageGo;
        IllMessageGo = m_Board.GetMessage(BkIll);
        if (!IllMessageGo.empty())
        {
            std::cout<<IllMessageGo[0]<<IllMessageGo[1]<<std::endl;
            ant.SetAntGoalX(IllMessageGo[0]);
            ant.SetAntGoalY(IllMessageGo[1]);
        }
    //}
