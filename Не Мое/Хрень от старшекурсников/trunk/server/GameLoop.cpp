...
void CGameLoop::UpdateMap() // расчет нового состояния карты после хода всех муравьев
{
    ...
    for (size_t antNumber = 0; antNumber < m_Map.GetNumberOfAnts(); ++antNumber)
    {
        ...
        // для начала считаем, что вся найденная еда и палочки попадает в муравейник
        ...
        // а вдруг победил?
        if (IsAntWinner(antNumber))
        {
            // объявляем ПОБЕДИТЕЛЕМ
            a.SetAntStatus(AsWinner);
            a.SetLastStepNumber(m_GameStep);
        }
        ...
        for (int row = y - r; row <= y + r; row++)
        {
            ...
            {
               a.SetVisibleCells(a.GetVisibleCells() + (char)m_Map.Cell(col, row));
...
{
    system("clear");
    std::cout  << "Шаг: " << m_GameStep << " Еды: " << m_FoodInHill
               <<  " На складе: " << m_FoodInStock
                << " Веточек: " << m_StickInHill
                << " Выращено деток: "   << m_ChildrenGrownup << std::endl;
    ...
    for (size_t antNumber = 0; antNumber < m_Map.GetNumberOfAnts(); ++antNumber)
    {
        ...
        if (!a.AntIsBanned() && !a.AntIsWinner())
        {
            std::cout  << "Муравей " << a.GetAntName()
                       ...
                       << ". Хотел пойти: " << MoveDirectionToString(a.GetAntMoveCommand())
                       << std::endl << std::flush;
