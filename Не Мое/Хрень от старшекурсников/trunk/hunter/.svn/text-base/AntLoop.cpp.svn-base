...
//        }
       ...
       // например, читаем сообщения о расположении еды
        ...
        if(!m_Board.GetMessage(BkLion).empty())
        {
        std::vector<int> lionMessage = m_Board.GetMessage(BkLion);

        ant.SetAntGoalX(lionMessage[0]);
        ant.SetAntGoalY(lionMessage[1]);
        m_Patrol = false;
        }
        ...
        if (m_Patrol)
        {
            if ((ant.GetAntPosX() == hillX - 2) && (ant.GetAntPosY() == hillY - 2))
            {
                direction = 0;
            }
            else if ((ant.GetAntPosX() == hillX + 2) && (ant.GetAntPosY() == hillY - 2))
            {
                direction = 1;
            }
            else if ((ant.GetAntPosX() == hillX + 2) && (ant.GetAntPosY() == hillY + 2))
            ...
            if (direction == 0)
            {
                ant.SetAntGoalX(hillX + 2);
                ant.SetAntGoalY(hillY - 2);
            }
            if (direction == 1)
            {
                ant.SetAntGoalX(hillX + 2);
