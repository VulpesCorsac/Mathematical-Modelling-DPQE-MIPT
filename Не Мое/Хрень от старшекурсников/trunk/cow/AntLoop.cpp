...
void CAntLoop::UpdateAnt(CAnt &ant)
{
    ...
    std::vector<AntType> GOGO = m_DynMap.Cell3D(ant.GetAntPosX(), ant.GetAntPosY());
    ...
    for (int y = 0; y <= m_Map.GetMapSizeY(); y++)
    {
        for (int x = 0; x <= m_Map.GetMapSizeX(); x++)
        {
            ...
            for(size_t deptha = 0; deptha < GOGO.size(); ++deptha)
            {
                if(GOGO[deptha] == AtCowboy)
                {
                  //  return true;
                ...
                // Ищем ковбоя
              //  if (m_DynMap.Cell3D(x, y) == AtCowboy)
               // {
                    ant.SetAntGoalX(x);
                    ant.SetAntGoalY(y);
                    RUN = true;
