...
CAntLoop::CAntLoop(): m_HillFound(false)
{
...
void CAntLoop::UpdateAnt(CAnt &ant)
{
        ...
        for (int rowNumber = 0; rowNumber < m_Map.GetMapSizeY(); ++rowNumber)
        {
            for (int colNumber = 0; colNumber < m_Map.GetMapSizeX(); ++colNumber)
            {
                ...
                {
                    if ( ( m_DynMap.Cell(colNumber, rowNumber)  == AtLion) && (m_HillFound) )
                    {
                        ant.SetAntGoalX(rowNumber);
                        ant.SetAntGoalY(colNumber);
                        std::cout << "следуем за львом" << std::endl;
                    }
            ...
            for (int y = 0; y <= m_Map.GetMapSizeY(); y++)
            {
                for (int x = 0; x <= m_Map.GetMapSizeX(); x++)
                {
                    ...
                    if (m_Map.Cell(x, y) == McHill)
                    {
                        ...
                        if((ant.GetAntPosX() == x)&&(ant.GetAntPosY() == y))
                        {
                            m_HillFound = true;
                        }
    ...
    if((ant.GetAntPosX() == ant.GetAntGoalX()) && (ant.GetAntPosY() == ant.GetAntGoalY()))
    {
