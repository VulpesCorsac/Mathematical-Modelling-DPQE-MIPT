...
void CAntLoop::UpdateAnt(CAnt &ant)
{
    ...
    if(((ant.GetAntPosX() == ant.GetAntGoalX()) && (ant.GetAntPosY() == ant.GetAntGoalY()))) //|| !ant.GetGoalReachable())
    {
        ...
        if(!m_HillFound)
        {
            SetGoalToNearestMapCell(McUnknown, ant);
            //  std::cout<< "target " << ant.GetAntGoalX() << "  " << ant.GetAntGoalY() << std::endl;
            // ant.SetGoalReachable(true);
        }
        bool ker = false;
        size_t numberOfMessage;

        if(m_Map.Cell(ant.GetAntPosX(), ant.GetAntPosY()) == McHill)
        {
            if(!m_Board.GetMessage(BkQueenCommand).empty())
            {
                ...
                {
                    if(StickMessageQueen[numberOfMessage] == 1) //если сообщение предназначено для рабочего
                    {
                        ker = true;
                        break;
                    }
                ...
                if (ker)
                {
                    ant.SetAntGoalX(StickMessageQueen[numberOfMessage + 1]);
                    ant.SetAntGoalY(StickMessageQueen[numberOfMessage + 2]);
                }
            }
            if(!ker)// если от королевы нету ничего для рабочего
            {
                ...
                if (m_Swap)
                {
                    // std::cout<<"laaa"<<stickMessageGo[0]<<"  "<<stickMessageGo[1]<<std::endl;
                    m_Swap = false;
